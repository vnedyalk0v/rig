...
rig_print_list() {
  ...
  local category id label kind package default_flag description version_strategy _versions _min_macos _notes
  category_filter=
  category_seen=no
  ...
  while IFS="$RIG_TSV_DELIMITER" read -r category id label kind package default_flag description version_strategy _versions _min_macos _notes; do
...
rig_collect_selected_tools() {
  ...
  local category id _id _label _kind _package default_flag _default_flag _description _version_strategy _versions _min_macos _notes
  select_arg=$1
  category_filter=$2
  seen='
'
...
"
  ...
  while IFS="$RIG_TSV_DELIMITER" read -r category id _label _kind _package default_flag _description _version_strategy _versions _min_macos _notes; do
    if [ "$category_filter" != "" ] && [ "$category" != "$category_filter" ]; then
      continue
    fi
...
rig_validate_selected_tools_macos() {
  local selected_tools current_version selected_id row id label min_macos
  local _category _kind _package _default_flag _description _version_strategy _versions _notes
  selected_tools=$1
  current_version=
  while IFS= read -r selected_id || [ "$selected_id" != "" ]; do
    ...
    IFS="$RIG_TSV_DELIMITER" read -r _category id label _kind _package _default_flag _description _version_strategy _versions min_macos _notes < <(printf '%s\n' "$row")
    if [ "$min_macos" = "" ]; then
      continue
    fi
...
rig_emit_brewfile_content() {
  local selected_tools selected_id row _category _id label kind package _default_flag _description _version_strategy _versions _min_macos _notes
  selected_tools=$1
  while IFS= read -r selected_id || [ "$selected_id" != "" ]; do
    if [ "$selected_id" = "" ]; then
      continue
    ...
    row=$(rig_lookup_tool "$selected_id")
    IFS="$RIG_TSV_DELIMITER" read -r _category _id label kind package _default_flag _description _version_strategy _versions _min_macos _notes < <(printf '%s\n' "$row")
    case "$kind" in
...
rig_emit_install_plan() {
  local selected_tools version_map format selected_id row id label kind package version_strategy versions selected_version _category _default_flag _description _min_macos _notes
  selected_tools=$1
  version_map=$2
  format=$3
  if [ "$format" = "file" ]; then
  ...
  while IFS= read -r selected_id || [ "$selected_id" != "" ]; do
    ...
    row=$(rig_lookup_tool "$selected_id")
    IFS="$RIG_TSV_DELIMITER" read -r _category id label kind package _default_flag _description version_strategy versions _min_macos _notes < <(printf '%s\n' "$row")
    case "$kind" in
...
rig_count_shell_edits_needed() {
  local selected_tools selected_id row version_strategy count _category _id _label _kind _package _default_flag _description _versions _min_macos _notes
  selected_tools=$1
  count=0
  while IFS= read -r selected_id || [ "$selected_id" != "" ]; do
    if [ "$selected_id" = "" ]; then
    ...
    row=$(rig_lookup_tool "$selected_id")
    IFS="$RIG_TSV_DELIMITER" read -r _category _id _label _kind _package _default_flag _description version_strategy _versions _min_macos _notes < <(printf '%s\n' "$row")
...
rig_count_homebrew_selected() {
  local selected_tools selected_id row kind count _category _id _label _package _default_flag _description _version_strategy _versions _min_macos _notes
  selected_tools=$1
  count=0
  while IFS= read -r selected_id || [ "$selected_id" != "" ]; do
    if [ "$selected_id" = "" ]; then
    ...
    row=$(rig_lookup_tool "$selected_id")
    IFS="$RIG_TSV_DELIMITER" read -r _category _id _label kind _package _default_flag _description _version_strategy _versions _min_macos _notes < <(printf '%s\n' "$row")
    case "$kind" in
      formula|cask|tap-formula|mas|vscode)
        count=$((count + 1))
        ;;
...
rig_resolve_plan_selections() {
  ...
  if ! rig_validate_selected_tools_macos "$selected_tools"; then
    return 1
  fi
